            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    if not validate_result.get("success"):
                        return {
                            "success": False,
                            "error": validate_result.get("error") or "兑换码校验失败",
                        }
                    if not validate_result.get("valid"):
                        return {
                            "success": False,
                            "error": validate_result.get("reason") or "兑换码无效",
                        }
                    code_meta = validate_result.get("redemption_code") or {}
                    pool_type = code_meta.get("pool_type", "normal")
                    is_virtual_welfare_code = bool(
                        code_meta.get("virtual_welfare_code")
                    )
                    ...
                    async with _team_locks[team_id_final]:
                        ...
                        try:
                            ...
                            reserve_result = (
                                await self.team_service.reserve_seat_if_available(
                                    ...
                                    if any(
                                        ...
                                        if seat_reserved:
                                            target_team.current_members = max(
                                                target_team.current_members - 1, 0
                                            )
                                            seat_reserved = False
                                        if (
                                            target_team.current_members
