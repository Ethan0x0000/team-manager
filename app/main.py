...
def normalize_token_refresh_interval(interval_minutes: int) -> int:
    return max(
        MIN_TOKEN_REFRESH_INTERVAL_MINUTES,
        min(MAX_TOKEN_REFRESH_INTERVAL_MINUTES, interval_minutes),
    )


def normalize_token_refresh_window(window_hours: int) -> int:
    return max(
        MIN_TOKEN_REFRESH_WINDOW_HOURS,
        min(MAX_TOKEN_REFRESH_WINDOW_HOURS, window_hours),
    )


def normalize_periodic_team_sync_interval_hours(interval_hours: int) -> int:
    return max(
        MIN_PERIODIC_TEAM_SYNC_INTERVAL_HOURS,
        min(MAX_PERIODIC_TEAM_SYNC_INTERVAL_HOURS, interval_hours),
    )


def normalize_periodic_team_sync_days(refresh_interval_days: int) -> int:
    return max(
        MIN_PERIODIC_TEAM_SYNC_DAYS,
        min(MAX_PERIODIC_TEAM_SYNC_DAYS, refresh_interval_days),
    )
...
async def configure_periodic_team_sync_job_from_settings() -> tuple[bool, int, int]:
    ...
    async with AsyncSessionLocal() as session:
        ...
        days_raw = await settings_service.get_setting(
            session, "periodic_team_sync_days", str(DEFAULT_PERIODIC_TEAM_SYNC_DAYS)
        )
...
async def scheduled_proactive_refresh():
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            stats = await team_service.proactive_refresh_tokens(
                session, refresh_window_hours=window_hours
            )
            logger.info(
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            stats = await team_service.sync_teams_due_for_periodic_refresh(
                session, refresh_interval_days=refresh_days
            )
            logger.info(
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        (
            ...
            periodic_interval,
            periodic_days,
        ) = await configure_periodic_team_sync_job_from_settings()
        if periodic_enabled:
            logger.info(
