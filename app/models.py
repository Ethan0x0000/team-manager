...
class Team(Base):
    ...
    status = Column(
        String(20), default="active", comment="状态: active/full/expired/error/banned"
    )
    account_role = Column(
        String(50), comment="账号角色: account-owner/standard-user 等"
    )
    device_code_auth_enabled = Column(
        Boolean, default=False, comment="是否开启设备代码身份验证"
    )
    ...
    deleted_at = Column(
        DateTime, nullable=True, default=None, comment="软删除时间，非 NULL 表示已删除"
    )

    # 关系
    team_accounts = relationship(
        "TeamAccount", back_populates="team", cascade="all, delete-orphan"
    )
    redemption_records = relationship(
        "RedemptionRecord", back_populates="team", cascade="all, delete-orphan"
    )
    email_mappings = relationship(
        "TeamEmailMapping", back_populates="team", cascade="all, delete-orphan"
    )
...
class TeamEmailMapping(Base):
    ...
    status = Column(
        ...
        default="invited",
        nullable=False,
        comment="状态: invited/joined/removed",
    )
    source = Column(
        String(20),
        default="sync",
        nullable=False,
        comment="来源: redeem/admin_add/sync/api",
    )
    last_seen_at = Column(DateTime, default=get_now, comment="最后一次确认该状态的时间")
    missing_sync_count = Column(
        Integer, default=0, nullable=False, comment="连续同步缺失次数"
    )
    created_at = Column(DateTime, default=get_now, comment="创建时间")
...
class RedemptionCode(Base):
    ...
    status = Column(
        String(20),
        default="unused",
        comment="状态: unused/used/expired/warranty_active",
    )
    created_at = Column(DateTime, default=get_now, comment="创建时间")
    ...
    warranty_expires_at = Column(
        DateTime, comment="质保到期时间(首次使用后根据质保时长计算)"
    )
    pool_type = Column(
        String(20), default="normal", comment="兑换池类型: normal/welfare"
    )
    reusable_by_seat = Column(Boolean, default=False, comment="是否可按席位重复使用")

    # 关系
    redemption_records = relationship(
        "RedemptionRecord", back_populates="redemption_code"
    )
...
class RedemptionRecord(Base):
    ...
    code = Column(
        String(32),
        ForeignKey("redemption_codes.code"),
        nullable=False,
        comment="兑换码",
    )
    ...
    redemption_code = relationship(
        "RedemptionCode", back_populates="redemption_records"
    )
