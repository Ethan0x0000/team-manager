...
class OAuthAuthorizeRequest(BaseModel):
    ...
    client_id: str = Field(
        "app_EMoamEEZ73f0CkXaXp7hrann", description="OAuth Client ID"
    )
    redirect_uri: str = Field(
        "http://localhost:1455/auth/callback", description="回调地址"
    )
    ...
    id_token_add_organizations: bool = Field(
        True, description="是否在 id_token 中附带组织信息"
    )
...
class OAuthCallbackParseRequest(BaseModel):
    ...
    client_id: Optional[str] = Field(
        "app_EMoamEEZ73f0CkXaXp7hrann", description="兜底 client_id"
    )
    redirect_uri: str = Field(
        "http://localhost:1455/auth/callback", description="回调地址"
    )
...
class TeamUpdateRequest(BaseModel):
    ...
    status: Optional[str] = Field(
        None, description="状态: active/full/expired/error/banned"
    )
...
async def welfare_dashboard(
    ...
    try:
        ...
        remaining_spots = await team_service.get_total_available_seats(
            db, pool_type="welfare"
        )
    ...
    try:
        ...
        if total_seats <= 0:
            return JSONResponse(
                ...
                content={
                    "success": False,
                    "error": "当前没有可用的福利车位，无法生成通用兑换码",
                },
            )

        current_welfare_code = (
            await settings_service.get_setting(db, "welfare_common_code", "") or ""
        ).strip()
        max_attempts = 10
        code = None
        ...
        await db.execute(
            ...
            .where(
                RedemptionCode.pool_type == "welfare",
                RedemptionCode.reusable_by_seat == True,
            )
...
async def delete_team(
    ...
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "error": "删除 Team 失败，请稍后重试"},
        )
...
async def team_import(
    ...
    try:
        pool_type = (
            "welfare" if (import_data.pool_type or "normal") == "welfare" else "normal"
        )
        ...
        if import_data.import_type == "single":
            ...
            if not any(
                [
                    import_data.access_token,
                    import_data.refresh_token,
                    import_data.session_token,
                ]
            ):
                return JSONResponse(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            async def progress_generator():
                async for status_item in team_service.import_team_batch(
                    text=import_data.content, db_session=db, pool_type=pool_type
                ):
            ...
            async def progress_generator():
                async for status_item in team_service.import_team_json(
                    json_text=import_data.content, db_session=db, pool_type=pool_type
                ):
                    yield json.dumps(status_item, ensure_ascii=False) + "\n"
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "error": "导入失败，请稍后重试"},
        )


@router.post("/oauth/openai/authorize")
async def create_openai_oauth_authorize_url(
    payload: OAuthAuthorizeRequest, current_user: dict = Depends(require_admin)
):
    ...
    try:
        ...
        if not client_id:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"success": False, "error": "client_id 不能为空"},
            )
...
async def parse_openai_oauth_callback(
    ...
    try:
        ...
        if not text:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"success": False, "error": "回调内容不能为空"},
            )
        ...
        if (
            ...
            and merged.get("state")
            and merged.get("state") != payload.expected_state
        ):
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"success": False, "error": "state 不匹配，请重新生成授权链接"},
            )
        ...
        if code and not access_token:
            ...
            if not exchange.get("success"):
                return JSONResponse(
                    status_code=status.HTTP_400_BAD_REQUEST, content=exchange
                )
...
async def team_members_list(
    ...
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "error": "获取成员列表失败，请稍后重试"},
        )
...
async def add_team_member(
    ...
    try:
        ...
        result = await team_service.add_team_member(
            team_id=team_id, email=member_data.email, db_session=db
        )
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "error": "添加成员失败，请稍后重试"},
        )
...
async def delete_team_member(
    ...
    try:
        ...
        result = await team_service.delete_team_member(
            team_id=team_id, user_id=user_id, db_session=db
        )
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "error": "删除成员失败，请稍后重试"},
        )
...
async def revoke_team_invite(
    ...
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "error": "撤回邀请失败，请稍后重试"},
        )
...
async def batch_refresh_teams(
    action_data: BulkActionRequest, current_user: dict = Depends(require_admin)
):
                ...
                for index, team_id in enumerate(team_ids, start=1):
                    ...
                    try:
                        result = await team_service.sync_team_info(
                            team_id, db_session, force_refresh=True
                        )
...
async def codes_list_page(
    ...
    try:
        ...
        logger.info(
            f"管理员访问兑换码列表页面, search={search}, status={status_filter}, per_page={per_page}"
        )
...
async def generate_codes(
    ...
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "error": "生成失败，请稍后重试"},
        )
...
async def delete_code(
    ...
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "error": "删除失败，请稍后重试"},
        )
...
async def cleanup_invalid_codes(
    ...
    try:
        result = await redemption_service.cleanup_invalid_codes(
            cleanup_data.codes, db, pool_type="normal"
        )
        status_code = (
            status.HTTP_200_OK if result["success"] else status.HTTP_400_BAD_REQUEST
        )
...
async def export_codes(
    ...
    try:
        ...
        # 获取所有兑换码 (导出不分页，传入大数量)
        codes_result = await redemption_service.get_all_codes(
            db, page=1, per_page=100000, search=search, pool_type="normal"
        )
        ...
        for row, code in enumerate(all_codes, start=1):
            ...
            worksheet.write(
                row,
                6,
                code.get("warranty_days", "-") if code.get("has_warranty") else "-",
                cell_format,
            )
...
async def records_page(
    ...
    try:
        ...
        records_result = await redemption_service.get_all_records(
            db, email=email, code=code, team_id=actual_team_id
        )
    ...
    try:
        ...
        return templates.TemplateResponse(
            ...
            "admin/settings/index.html",
            {
                ...
                "log_level": log_level,
                "webhook_url": await settings_service.get_setting(
                    db, "webhook_url", ""
                ),
                "low_stock_threshold": await settings_service.get_setting(
                    db, "low_stock_threshold", "10"
                ),
                "api_key": await settings_service.get_setting(db, "api_key", ""),
                "token_refresh_interval_minutes": await settings_service.get_setting(
                    db, "token_refresh_interval_minutes", "30"
                ),
                "token_refresh_window_hours": await settings_service.get_setting(
                    db, "token_refresh_window_hours", "2"
                ),
                "token_refresh_client_id": await settings_service.get_setting(
                    db, "token_refresh_client_id", ""
                ),
                "periodic_team_sync_enabled": await settings_service.get_setting(
                    db, "periodic_team_sync_enabled", "true"
                ),
                "periodic_team_sync_interval_hours": await settings_service.get_setting(
                    db, "periodic_team_sync_interval_hours", "12"
                ),
                "periodic_team_sync_days": await settings_service.get_setting(
                    db, "periodic_team_sync_days", "7"
                ),
                "default_team_max_members": await settings_service.get_setting(
                    db, "default_team_max_members", "5"
                ),
                "cliproxyapi_base_url": await settings_service.get_setting(
                    db, "cliproxyapi_base_url", ""
                ),
                "cliproxyapi_api_key": await settings_service.get_setting(
                    db, "cliproxyapi_api_key", ""
                ),
...
class TeamImportSettingsRequest(BaseModel):
    ...
    default_team_max_members: int = Field(
        5, ge=1, le=100, description="新导入 Team 的默认总席位"
    )
...
class WarrantyExpirationSettingsRequest(BaseModel):
    ...
    expiration_mode: Literal["first_use", "refresh_on_redeem"] = Field(
        DEFAULT_WARRANTY_EXPIRATION_MODE, description="质保时长计算模式"
    )
...
class UiThemeSettingsRequest(BaseModel):
    ...
    theme: Literal["ocean", "warm"] = Field(
        DEFAULT_UI_THEME, description="系统配色主题"
    )
...
async def update_ui_theme_settings(
    ...
    try:
        ...
        if success:
            return JSONResponse(
                content={"success": True, "message": "系统配色已保存", "theme": theme}
            )
...
async def announcement_page(
    ...
    try:
        ...
        enabled_raw = await settings_service.get_setting(
            db, "announcement_enabled", "false"
        )
        announcement_enabled = str(enabled_raw).lower() in {"1", "true", "yes", "on"}
        announcement_markdown = await settings_service.get_setting(
            db, "announcement_markdown", ""
        )
...
async def update_proxy_config(
    ...
    try:
        ...
        logger.info(
            f"管理员更新代理配置: enabled={proxy_data.enabled}, proxy={proxy_data.proxy}"
        )
        ...
        success = await settings_service.update_proxy_config(
            db, proxy_data.enabled, proxy_data.proxy.strip() if proxy_data.proxy else ""
        )
...
async def update_webhook_settings(
    ...
    try:
        ...
        logger.info(
            f"管理员更新 Webhook/API 配置: url={webhook_data.webhook_url}, threshold={webhook_data.low_stock_threshold}"
        )
...
async def update_team_import_settings(
    ...
    try:
        ...
        if success:
            return JSONResponse(
                content={"success": True, "message": "Team 导入配置已保存"}
            )
